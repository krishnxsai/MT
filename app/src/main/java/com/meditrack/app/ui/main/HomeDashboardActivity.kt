...
import com.meditrack.app.databinding.ActivityHomeDashboardBinding
import com.meditrack.app.ui.auth.LoginActivity
import com.meditrack.app.ui.chat.ChatListActivity
import com.meditrack.app.ui.chat.ChatListViewModel
import com.meditrack.app.ui.appointment.AppointmentsListActivity
import com.meditrack.app.ui.healthlog.HealthLogListActivity
import com.meditrack.app.ui.healthlog.HealthLogViewModel
import com.meditrack.app.ui.insights.HealthInsightsActivity
import com.meditrack.app.ui.insights.RiskDashboardActivity
import com.meditrack.app.ui.medicine.MedicinesListActivity
...
class HomeDashboardActivity : AppCompatActivity() {
    ...
    private fun setupClickListeners() {
        ...
        binding.healthInsightsCard.setOnClickListener {
            startActivity(Intent(this, com.meditrack.app.ui.insights.HealthInsightsActivity::class.java))
        }
    ...
    private fun observeRiskCard() {
        homeDashboardViewModel.riskCardState.observe(this) { state ->
            if (state == null) {
                binding.riskScoreCard.visibility = View.GONE
                return@observe
            }
            binding.riskScoreCard.visibility = View.VISIBLE
            binding.riskMiniScore.text = state.overallScore.toString()
            binding.riskMiniGauge.setProgressCompat(state.overallScore, true)

            val (label, badgeColor, gaugeColor) = when (state.category) {
            ...
            binding.riskSummaryText.text = when {
                state.factorCount == 0 -> "All clear — tap for details"
                state.factorCount == 1 -> "1 factor detected — tap for details"
                else -> "${state.factorCount} factors detected — tap for details"
            }
        }
    }

    private fun observeAdherenceStats() {
        homeDashboardViewModel.adherenceStats.observe(this) { stats ->
            if (stats == null) return@observe
            binding.progressText.text = "${stats.taken}/${stats.totalScheduled} taken"
            binding.medicineProgress.max = if (stats.totalScheduled > 0) stats.totalScheduled else 1
            binding.medicineProgress.progress = stats.taken
        }
    }
